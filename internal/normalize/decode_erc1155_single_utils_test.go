...
func TestDecodeERC1155Single(t *testing.T) {
    ...
    l := eth.Log{TxHash: "0x1", Index: 1, Address: "0xdead", Topics: []string{"0xc3d58168", "0x"+strings.Repeat("0",64), padAddr(from), padAddr(to)}, DataHex: data}
    transfers, approvals := DecodeTokenEvents([]eth.Log{l})
    if len(approvals) != 0 || len(transfers) != 1 { t.Fatalf("unexpected counts") }
...
func TestSplitDataWordsEmpty(t *testing.T) {
    if words := splitDataWords("0x"); len(words) != 0 { t.Fatalf("expected empty words") }
}
