...
func DecodeTokenEvents(logs []eth.Log) (transfers []TokenTransferRow, approvals []ApprovalRow) {
    for _, l := range logs {
        ...
        case strings.HasPrefix(t0, topicApprovalForAllPrefix):
            ...
            isForAll := uint8(0)
            if strings.HasSuffix(strings.ToLower(l.DataHex), strings.Repeat("0", 63)+"1") { isForAll = 1 }
            approvals = append(approvals, ApprovalRow{
                EventUID: fmt.Sprintf("%s:%d", l.TxHash, l.Index),
                TxHash:   l.TxHash,
                LogIndex: l.Index,
...
func parseERC1155Batch(data string) (ids []string, vals []string) {
    ...
    readArray := func(offBytes int) []string {
        ...
        idx := offBytes * 2
        if idx%64 != 0 || idx < 0 || idx+64 > len(d) { return nil }
        // length at offset
