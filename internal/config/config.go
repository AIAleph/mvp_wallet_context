...
func RedactDSN(s string) string {
    ...
    if u, err := url.Parse(s); err == nil {
        if u.User != nil {
            ...
            if name != "" { u.User = url.UserPassword(name, "***") } else { u.User = url.User("***") }
            return u.String()
        }
...
func Load() Config {
    return Config{
        ...
        SyncConfirmations: parseIntEnv("SYNC_CONFIRMATIONS", 12),
        BatchBlocks:       parseIntEnv("BATCH_BLOCKS", 5000),
        RateLimit:         parseIntEnv("RATE_LIMIT", 0),
        RedisURL:          env("REDIS_URL", ""),
        EmbeddingModel:    env("EMBEDDING_MODEL", ""),
        Timeout:           parseDurEnv("INGEST_TIMEOUT", 30*time.Second),
