...
func TestHTTPProvider_BlockNumber(t *testing.T) {
    ...
    if err != nil || n != 42 { t.Fatalf("bn=%d err=%v", n, err) }
}
...
func TestHTTPProvider_TraceFilterPaging(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(out) != 2 { t.Fatalf("expected 2 traces, got %d", len(out)) }
...
func TestHTTPProvider_BlockTimestampDirect(t *testing.T) {
    ...
    if err != nil || ts != 42000 { t.Fatalf("ts=%d err=%v", ts, err) }
}
...
func TestHTTPProvider_RpcErrorAndNoRetryOn400(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1; hp.maxRetries = 2 }
...
func TestHTTPProvider_BlockTimestampCache(t *testing.T) {
    ...
    for i := 0; i < 2; i++ { _, _ = p.BlockTimestamp(context.Background(), 16) }
    if calls != 1 { t.Fatalf("expected 1 rpc call, got %d", calls) }
}
...
func TestHTTPProvider_429RetryThenSuccess(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1; hp.maxRetries = 3 }
    n, err := p.BlockNumber(context.Background())
    if err != nil || n != 42 || calls != 3 { t.Fatalf("n=%d err=%v calls=%d", n, err, calls) }
}

func TestNewHTTPProvider_EmptyEndpointAndDefaultClient(t *testing.T) {
    if _, err := NewHTTPProvider("", nil); err == nil { t.Fatal("expected error for empty endpoint") }
    if p, err := NewHTTPProvider("http://unit-test", nil); err != nil || p == nil { t.Fatalf("new http provider err=%v", err) }
}
...
func TestTraceBlockZeroResults(t *testing.T) {
    client := &http.Client{Transport: rtFunc(func(r *http.Request) (*http.Response, error) {
        var req map[string]any
        _ = json.NewDecoder(r.Body).Decode(&req)
        if req["method"] == "trace_filter" { return mkResp([]any{}), nil }
        return mkResp(nil), nil
    })}
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1 }
    out, err := p.TraceBlock(context.Background(), 1, 10, "0xdead")
    if err != nil || len(out) != 0 { t.Fatalf("out=%v err=%v", out, err) }
}
...
func TestHTTPProvider_GetLogsTopicsVariationsAndTsError(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(out) != 2 { t.Fatalf("expected 2 logs, got %d", len(out)) }
    // First ts remains 0 due to error; second enriched
    if out[0].TsMillis != 0 || out[1].TsMillis == 0 { t.Fatalf("ts enrichment mismatch: %+v", out) }
}
...
func TestHTTPProvider_RetryBackoff(t *testing.T) {
    ...
    client := &http.Client{Transport: rtFunc(func(r *http.Request) (*http.Response, error) {
        var req map[string]any
        _ = json.NewDecoder(r.Body).Decode(&req)
        if req["method"] != "eth_blockNumber" { return mkResp(nil), nil }
        calls++
        if calls < 3 {
    ...
    p, err := NewHTTPProvider("http://unit-test", client)
    if err != nil { t.Fatal(err) }
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1; hp.maxRetries = 5 }
    n, err := p.BlockNumber(context.Background())
    if err != nil || n != 42 { t.Fatalf("bn=%d err=%v", n, err) }
...
func TestHTTPProvider_DoErrorThenSuccess(t *testing.T) {
    ...
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1; hp.maxRetries = 2 }
    n, err := p.BlockNumber(context.Background())
    if err != nil || n != 42 || calls != 2 { t.Fatalf("n=%d err=%v calls=%d", n, err, calls) }
}
