...
func TestHTTPProvider_Call_JSONDecodeError(t *testing.T) {
    ...
    if hp, ok := p.(*httpProvider); ok { hp.maxRetries = 1; hp.backoffBase = 1 }
    var out any
    if err := p.(*httpProvider).call(context.Background(), "eth_blockNumber", nil, &out); err == nil {
        t.Fatal("expected decode error")
    }
    if calls == 0 { t.Fatal("expected at least one call") }
}
...
func TestHTTPProvider_500RetryThenSuccess(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.maxRetries = 2; hp.backoffBase = 1 }
...
func TestHTTPProvider_BlockNumber_BadHex(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if _, err := p.BlockNumber(context.Background()); err == nil { t.Fatal("expected hex parse error") }
}

func TestHexToUint64_Invalid(t *testing.T) {
    if _, err := hexToUint64("bad"); err == nil { t.Fatal("expected error") }
}
...
func TestHTTPProvider_BlockNumber_CallErrorPath(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.maxRetries = 0 }
    if _, err := p.BlockNumber(context.Background()); err == nil { t.Fatal("expected call error") }
}
...
func TestHTTPProvider_BlockTimestamp_HexError(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if _, err := p.BlockTimestamp(context.Background(), 123); err == nil { t.Fatal("expected hex error") }
}
...
func TestHTTPProvider_ContextCancelDuringBackoff(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.maxRetries = 2; hp.backoffBase = 2 * time.Millisecond }
    ...
    if calls != 1 { t.Fatalf("expected 1 call before cancel, got %d", calls) }
}
...
func TestHTTPProvider_GetLogs_TopicsParamShapes(t *testing.T) {
    client := &http.Client{Transport: rtFunc(func(r *http.Request) (*http.Response, error) {
        ...
        if req["method"] == "eth_getLogs" {
            ...
            if !(topics[0] == nil) { t.Fatalf("expected nil in first topic group, got %v", topics[0]) }
            if s, ok := topics[1].(string); !ok || s != "0x01" { t.Fatalf("expected single string in second, got %T %v", topics[1], topics[1]) }
            if arr, ok := topics[2].([]any); !ok || len(arr) != 2 { t.Fatalf("expected array in third, got %T %v", topics[2], topics[2]) }
            // Return empty logs
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if hp, ok := p.(*httpProvider); ok { hp.backoffBase = 1 }
    out, err := p.GetLogs(context.Background(), "0xdead", 1, 2, [][]string{nil, []string{"0x01"}, []string{"0x02","0x03"}})
    if err != nil { t.Fatal(err) }
    if len(out) != 0 { t.Fatalf("expected no logs, got %d", len(out)) }
}
...
func TestHTTPProvider_GetLogs_CallError(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if _, err := p.GetLogs(context.Background(), "0x", 1, 2, nil); err == nil { t.Fatal("expected getLogs call error") }
}
...
func TestHTTPProvider_TraceBlock_CallError(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if _, err := p.TraceBlock(context.Background(), 1, 2, "0x"); err == nil { t.Fatal("expected trace_filter call error") }
}
...
func TestHTTPProvider_TraceBlock_AfterIncrementPaging(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(out) != 1000 { t.Fatalf("expected 1000 traces, got %d", len(out)) }
}
...
func TestHTTPProvider_GetLogs_TimestampEnrichmentError(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(out) != 1 || out[0].TsMillis != 0 { t.Fatalf("expected TsMillis=0 on enrichment error, got %+v", out) }
}
...
func TestHTTPProvider_TraceBlock_TimestampEnrichmentError(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if len(out) != 1 || out[0].TsMillis != 0 { t.Fatalf("expected TsMillis=0 on enrichment error: %+v", out) }
}
...
func TestHTTPProvider_BlockTimestamp_CallError(t *testing.T) {
    ...
    p, _ := NewHTTPProvider("http://unit-test", client)
    if _, err := p.BlockTimestamp(context.Background(), 1); err == nil { t.Fatal("expected call error") }
}
