...
func (p provHeadOK) GetLogs(ctx context.Context, address string, from, to uint64, topics [][]string) ([]eth.Log, error) {
	return nil, nil
}
...
func (p provLogsErr) GetLogs(ctx context.Context, address string, from, to uint64, topics [][]string) ([]eth.Log, error) {
...
func (provOnlyTrace) GetLogs(context.Context, string, uint64, uint64, [][]string) ([]eth.Log, error) {
	return nil, nil
}
...
func TestDelta_ProviderError_AndDefaultBatch(t *testing.T) {
	...
	if err := ing2.Delta(context.Background()); err != nil {
		t.Fatal(err)
	}
}
...
func TestProcessRange_ErrorPaths_TokenApprovalsAndTraces(t *testing.T) {
	...
	if err := ing.processRange(context.Background(), 1, 1); err == nil {
		t.Fatal("expected token transfers insert error")
	}
	...
	if err := ing2.processRange(context.Background(), 1, 1); err == nil {
		t.Fatal("expected approvals insert error")
	}
