...
func (p *tsProv) BlockTimestamp(ctx context.Context, block uint64) (int64, error) { p.calls++; return int64(block)*1000, nil }
func (p *tsProv) GetLogs(ctx context.Context, address string, from, to uint64, topics [][]string) ([]eth.Log, error) { return nil, nil }
func (p *tsProv) TraceBlock(ctx context.Context, from, to uint64, address string) ([]eth.Trace, error) { return nil, nil }
...
func TestGetBlockTs_NoProvider(t *testing.T) {
    ing := New("0x", Options{})
    if _, ok := ing.getBlockTs(context.Background(), 10); ok { t.Fatal("expected false with no provider") }
}
