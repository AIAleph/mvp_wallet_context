...
func (p *fakeProv) BlockTimestamp(ctx context.Context, block uint64) (int64, error) {
	p.tsCalls++
...
func (p *fakeProv) Transactions(ctx context.Context, address string, from, to uint64) ([]eth.Transaction, error) {
	return nil, nil
}
...
func TestBackfillDeltaAndTsCache(t *testing.T) {
	...
	p.logs = []struct {
		from, to uint64
		items    []string
	}{{from: 10, to: 10, items: []string{"l1"}}, {from: 11, to: 11, items: []string{"l2"}}}
	p.traces = []struct {
		from, to uint64
		items    []string
	}{{from: 10, to: 10, items: []string{"t1"}}}
	...
	if callsAfterBackfill == 0 {
		t.Fatal("expected timestamp calls")
	}
